...
# VeRPO-style dual advantage: A = A_traj + beta * A_turn.
# NOTE: Do not place inline comment-only lines inside the continued command
# below. In bash, that can terminate argument continuation unexpectedly.
"${PYTHON_BIN}" -m verl.trainer.main_ppo \
  algorithm.adv_estimator=verpo \
  algorithm.norm_adv_by_std_in_grpo=False \
  +algorithm.verpo_turn_beta=1.0 \
  +algorithm.verpo_turn_norm_by_std=False \
  ...
  +reward.custom_reward_function.reward_kwargs.difficulty_alpha=2.0 \
  +reward.custom_reward_function.reward_kwargs.stats_momentum=0.9 \
  +reward.custom_reward_function.reward_kwargs.default_pass_rate=0.5 \
  +reward.custom_reward_function.reward_kwargs.density_sigma_floor=0.001 \
  +reward.custom_reward_function.reward_kwargs.dense_reward_weight=1.0 \
  +reward.custom_reward_function.reward_kwargs.traj_reward_weight=1.0 \
  +reward.custom_reward_function.reward_kwargs.efficiency_gamma=0.995 \
  +reward.custom_reward_function.reward_kwargs.efficiency_mode=response_tokens \
